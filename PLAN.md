...
with SIA 410 symbols / SIA 410/1 colours (schema-layout.ts, schema-view.tsx), click to edit. Product data:
...
`Berechnungsvorlagen/Lüftung KWL/Zehnder Daten/digitalisiert/` (gitignored; vector curves calibrated on the grid) – Zehnder has priority, generic spiro ducts (Darcy–Weisbach) as
fallback; product keys are referenced by saved networks, keep them stable. Device check from the datasheet
(max. external pressure line, power by bilinear fit of the measurement table → SPI); workbook devices map to their
datasheet equivalent (…-st → datasheet) and the datasheet check has priority in the dwelling device tab too.
Dwellings served by a system take its external pressures. Quantities → LV group (IGH Zehnder catalogue positions by
